 ...
 * Ex: "java -cp mysql-connector-java-8.0.19.jar:json-simple-1.1.1.jar Bank.java"
 *
 */
...
public class Bank {
    static final String DB_ADDRESS = "jdbc:mysql://127.0.0.1:3306/cs160";
    static final String DB_USERNAME = "cs160";
     ...
     * @param firstName The new user's first name (max length: 30 characters, cannot contain ' " ; \)
     * @param lastName  The new user's last name (max length: 30 characters, cannot contain ' " ; \)
     * @param password  The new user's password (max length: 30 characters, cannot contain ' " ; \)
     * @param address  The new user's address (max length: 100 characters, cannot contain ' " ; \)
    ...
    public static String registerUser(String firstName, String lastName, String password, String address){
        try{
            //first name can be a maximum of 30 valid characters
            if(firstName.length() > 30 || !validString(firstName)) return "{\"Error\":true,\"Error Message\":\"Unable to create user: First name cannot be more than 30 characters or use apostrophes, apostrophes, quotes, backslashes, or semicolons\"}";
            //last name can be a maximum of 30 valid characters
            if(lastName.length() > 30 || !validString(lastName)) return "{\"Error\":true,\"Error Message\":\"Unable to create user: Last name cannot be more than 30 characters or use apostrophes, quotes, backslashes, or semicolons\"}";
            //password name can be a maximum of 30 valid characters
            if(password.length() > 30 || !validString(password)) return String.format("{\"Error\":true,\"Error Message\":\"Unable to create user (%s %s): Password cannot be more than 30 characters or use apostrophes, quotes, backslashes, or semicolons\"}", firstName, lastName);
            //address can be a maximum of 100 valid characters
            if(address.length() > 100 || !validString(address)) return String.format("{\"Error\":true,\"Error Message\":\"Unable to create user (%s %s): Address cannot be more than 100 characters or use apostrophes, quotes, backslashes, or semicolons\"}", firstName, lastName);
     ...
     * @param newFirstName  The customer's new first name (max length: 30 characters, cannot contain ' " ; \)
     * @param newLastName  The customer's new last name (max length: 30 characters, cannot contain ' " ; \)
    ...
    public static String changeName(int customerID, String password, String newFirstName, String newLastName){
        try{
            //first name can be a maximum of 30 valid characters
            if(newFirstName.length() > 30 || !validString(newFirstName)) return String.format("{\"Error\":true,\"Error Message\":\"Unable to change name for User #%d: First name cannot be more than 30 characters or use apostrophes, quotes, backslashes, or semicolons\"}", customerID);
            //last name can be a maximum of 30 valid characters
            if(newLastName.length() > 30 || !validString(newLastName)) return String.format("{\"Error\":true,\"Error Message\":\"Unable to change name for User #%d: Last name cannot be more than 30 characters or use apostrophes, quotes, backslashes, or semicolons\"}", customerID);
     ...
     * @param customerID The customer's user ID (must be valid)
     * @param password  The customer's password (must match user ID)
     * @param newAddress  The customer's new address (max length: 100 characters, cannot contain ' " ; \)
    ...
    public static String changeAddress(int customerID, String password, String newAddress){
        try{
            ...
            //address can be a maximum of 100 valid characters
            if(newAddress.length() > 100 || !validString(newAddress)) return String.format("{\"Error\":true,\"Error Message\":\"Unable to change address for User #%d: Address cannot be more than 100 characters or use apostrophes, quotes, backslashes, or semicolons\"}", customerID);
     ...
     * @param newPassword  The customer's new password (max length: 30 characters, cannot contain ' " ; \)
    ...
    public static String changePassword(int customerID, String oldPassword, String newPassword){
        try{
            ...
            //password can be a maximum of 30 valid characters
            if(newPassword.length() > 30 || !validString(newPassword)) return String.format("{\"Error\":true,\"Error Message\":\"Unable to change password for User #%d: Password cannot be more than 30 characters or use apostrophes, quotes, backslashes, or semicolons\"}", customerID);
     ...
     * @param customerID The customer's user ID (must be valid)
     * @param password  The customer's password (must match user ID)
     * @param accountName  The new account's name (max length: 50 characters, cannot contain ' " ; \)
    ...
    public static String openAccount(int customerID, String password, String accountName, String type, double initialDeposit){
        try{
            ...
            //account name can be a maximum of 50 valid characters
            if(accountName.length() > 50 || !validString(accountName)) return String.format("{\"Error\":true,\"Error Message\":\"Unable to create account for User #%d: Account name cannot be more than 50 characters or use apostrophes, quotes, backslashes, or semicolons\"}", customerID);
            ...
            String insert = String.format("INSERT INTO Accounts (AccountName, Balance, Type, CustomerID) VALUES ('%s', 0, '%s', %d);", accountName, type, customerID);
            stmt.execute(insert);
            //gets account ID
            String getID = String.format("SELECT MAX(AccountID) From Accounts WHERE AccountName = '%s' AND Balance = 0 AND Type = '%s' AND CustomerID = %d;", accountName, type, customerID);
            int accountID = 0;
            rs = stmt.executeQuery(getID);
            ...
            //deposit the initial deposit into the account
            deposit(customerID, password, accountID, initialDeposit);
            ...
            while(rs.next()) return String.format("{\"Error\":false,\"Account ID\":%d,\"User ID\":%d,\"Account Name\":\"%s\",\"Account Type\":\"%s\",\"Balance\":\"$%s\"}", accountID, customerID, accountName, expandedType, rs.getString(1));
     ...
     * @param newAccountName  The account's new name (max length: 50 characters, cannot contain ' " ; \)
    ...
    public static String changeAccountName(int customerID, String password, int accountID, String newAccountName){
        try{
            ...
            //new account name can be a maximum of 50 valid characters
            if(newAccountName.length() > 50 || !validString(newAccountName)) return String.format("{\"Error\":true,\"Error Message\":\"Unable to change name for Account #%d: Account name cannot be more than 50 characters or use apostrophes, quotes, backslashes, or semicolons\"}", accountID);
    ...
    public static String executeAPICall(String inputJSONString){
        try{
            ...
            if(function.equals("Register User")){
                ...
                Double initialDeposit =Double.parseDouble(inputJSON.get("Initial Deposit").toString());
                ...
                String password = (String) inputJSON.get("Password");
                Long accountID = (Long) inputJSON.get("Account ID");
                Double amount = Double.parseDouble(inputJSON.get("Amount").toString());
                ...
                Long customerID = (Long) inputJSON.get("User ID");
                String password = (String) inputJSON.get("Password");
                Boolean includeInactive = inputJSON.get("Include Inactive").toString() == "true";
            ...
            else return "{\"Error\":true,\"Error Message\":\"Invalid Function Name\"}";
        }
        catch (Exception e){
    ...
    */
    ...
    /**
     * Finds whether a string is valid (e.g does not contain ' " ; \)
    ...
    /**
     * Listens to the Intermediate Database Table: APIRequests
     * Upon update to this table, incoming API calls are executed and their outputs are put into the table
     * @param args Not used
     */
    public static void main(String[] args) throws ClassNotFoundException, SQLException{
        ...
        Bank bank = new Bank();
        System.out.println();
        System.out.println("Listening to APIRequests Table for new API calls");
        System.out.println();
        ...
        while(true){
            ...
            for(APIRequest req : newRequests){
                ...
                if(completeCount > 0) System.out.printf("API Request #%d Was Already Serviced\n", req.apiID);
                    ...
                    System.out.println(insertOutputJSON);
